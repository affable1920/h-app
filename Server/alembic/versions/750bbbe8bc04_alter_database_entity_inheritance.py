...
def upgrade() -> None:
    ...
    op.add_column("clinic", sa.Column(
        "owner", sa.UUID, sa.ForeignKey("user.id")))
    ...
    op.add_column("slot", sa.Column("mode", type_=sa.Enum(
        Mode, native_enum=False, create_constraint=True), server_default="IN_PERSON"))
    ...
    op.alter_column("doctor", column_name="status", type_=sa.Enum(
        Status, native_enum=False, create_constraint=True), server_default="UNKNOWN")
    ...
    op.add_column("appointment", sa.Column("status", type_=sa.Enum(
        AppointmentStatus, native_enum=False, check_constraint=True), server_default="ACTIVE"))
...
def downgrade() -> None:
    """Downgrade schema."""
    for col in user_drop_cols:
        op.add_column("user", sa.Column(name=col))

    op.drop_column("user", "name")

    op.add_column("doctor", sa.Column(
        "currently_available", sa.Boolean, default=True))
    op.alter_column("doctor", column_name="status", type_=sa.Enum(Status),
                    server_default="'AVAILABLE'", default=Status.AVAILABLE)
    ...
    op.alter_column("slot", column_name="mode", type_=sa.Enum(Mode))

    op.alter_column("user", column_name="role", type_=sa.Enum(UserRole))
    op.alter_column("doctor", column_name="status", type_=sa.Enum(Status))
    ...
    op.alter_column("appointments", "status", type_=sa.Enum(
        AppointmentStatus), server_default="'ACTIVE'")
